...
public class CameraUtils {
	...
	public static Camera.Size getWengOptimalPreviewSize(int displayOrientation,
			int width, int height, Camera.Parameters parameters) {
		double targetRatio = (double) width / height;
		...
		if (optimalSize == null) {
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
				optimalSize = parameters.getPreferredPreviewSizeForVideo();
			} else {
				optimalSize = null;
			}
		}
	...
	@TargetApi(Build.VERSION_CODES.HONEYCOMB)
	public static Camera.Size getWengOptimalPreviewSize2(
			int displayOrientation, int width, int height,
	...
	public static Camera.Size getLargestPictureSize(CameraHost host,
			Camera.Parameters parameters, boolean enforceProfile) {
		...
		for (Camera.Size size : parameters.getSupportedPictureSizes()) {
			...
			if (!enforceProfile
					|| (size.height <= host.getDeviceProfile()
							.getMaxPictureHeight() && size.height >= host
							.getDeviceProfile().getMinPictureHeight())) {
				if (result == null) {
					result = size;
				} else {
