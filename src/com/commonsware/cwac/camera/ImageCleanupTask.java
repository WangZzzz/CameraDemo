...
//		Log.d("wenhao", "applyMatrix... = " + applyMatrix);
		if (applyMatrix) {
			...
			try {
				...
				if (imageOrientation != 0) {
					matrix = rotate((matrix == null ? new Matrix() : matrix),
							imageOrientation);
				}
			} catch (IOException e) {
...
//				Log.d("wenhao", original.getWidth()+","+original.getHeight());
				cleaned = Bitmap
	...
	private static int calculateHeapSize(Context ctxt) {
		ActivityManager am = (ActivityManager) ctxt
