...
public class CameraView extends ViewGroup implements AutoFocusCallback {
		...
		if (width > 0 && height > 0) {
			if (camera != null) {
				...
				try {
					...
					// Log.d("wenhao", "getHost().getRecordingHint() = " +
					// getHost().getRecordingHint());
	...
	public void takePicture(final PictureTransaction xact) {
		if (inPreview) {
			if (isAutoFocusing) {
				...
				Camera.Parameters pictureParams = previewParams;
	...
	private class PictureTransactionCallback implements Camera.PictureCallback {
		...
		public void onPictureTaken(byte[] data, Camera camera) {
			...
			if (data != null) {
				new ImageCleanupTask(getContext(), data, cameraId, xact)
						.start();
			}
